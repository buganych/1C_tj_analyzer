"""End-to-end pipeline tests with in-memory events."""

from datetime import datetime, timedelta

from tlock_analyzer.analysis.pipeline import run_analysis
from tlock_analyzer.models import QueryFilters, TjEvent
from tlock_analyzer.sources.memory import MemoryLogSource
...
def test_pipeline_finds_full_match_by_log_id():
...
def test_pipeline_optional_date_filter():
    source = _build_scenario()
    filters = QueryFilters(
        log_ids=[LOG_ID],
        time_from=datetime(2026, 5, 27, 10, 54, 0),
        time_to=datetime(2026, 5, 27, 10, 55, 0),
    )
    assert len(run_analysis(source, filters).victims) == 1

    filters_outside = QueryFilters(
        log_ids=[LOG_ID],
        time_from=datetime(2026, 5, 28, 0, 0, 0),
        time_to=datetime(2026, 5, 28, 1, 0, 0),
    )
    assert len(run_analysis(source, filters_outside).victims) == 0
