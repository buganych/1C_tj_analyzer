...
app = typer.Typer(help="Analyze 1C TLOCK wait events and find lock culprits")
console = Console()
...
def main(
    ...
    if output in (OutputType.json, OutputType.both):
        console.print(_render_json(result, labels=TLOCK_LABELS))
    if output in (OutputType.text, OutputType.both):
        if output == OutputType.both:
            console.print("\n" + "=" * 40 + " TEXT REPORT " + "=" * 40 + "\n")
        console.print(_render_text(result, labels=TLOCK_LABELS))
